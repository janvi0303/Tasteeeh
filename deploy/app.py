...
if bruh == True and len(ans1) > 0:
    finallist = st.selectbox("Our Choices", ans1)
elif bruh == True and len(ans1) == 0:
    st.warning("No dishes match your preference!")
...
def food_recommendation(Food_Name):
    ...
    if not isinstance(Food_Name, str) or not Food_Name:
        return []
    FoodList = food[food['Name'].str.contains(Food_Name, regex=False)]  
    if len(FoodList):        
...
if bruh == True and len(display) > 0:
